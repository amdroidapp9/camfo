...
package com.ap.cropper.cropwindow;
...
import com.ap.cropper.cropwindow.handle.Handle;
import com.ap.cropper.util.AspectRatioUtil;
import com.ap.cropper.util.HandleUtil;
...
public class CropOverlayView extends View {
	...
	public static Paint mCornerPaint;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			Log.w("Touch", "ACTION_DOWN");
			onActionDown(centerX, centerY);
		...
		case MotionEvent.ACTION_CANCEL:
			Log.w("Touch", "ACTION_CANCEL");
			getParent().requestDisallowInterceptTouchEvent(false);
			onActionUp();
			return true;

		case MotionEvent.ACTION_MOVE:
			Log.w("Touch", "ACTION_MOVE");
			onActionMove(event.getX(), event.getY());
	...
	private void initCropWindow(Rect bitmapRect) {
		...
		if (mFixAspectRatio) {
			...
			final float horizontalPadding = (bitmapRect.width() - 300) / 2;
			final float verticalPadding = (bitmapRect.height() - 300) / 2;

			centerX = bitmapRect.width() / 2;
			centerY = bitmapRect.height() / 2;
	...
	private void onActionDown(float x, float y) {
		...
		if (mPressedHandle == null) {
			// left = mBitmapRect.left;
			// top = mBitmapRect.top;
			// right = mBitmapRect.right;
			// bottom = mBitmapRect.bottom;
			// mPressedHandle = HandleUtil.getPressedHandle(
			// mBitmapRect.exactCenterX(), mBitmapRect.exactCenterY(),
			// left, top, right, bottom, mHandleRadius);
			return;
		}
		// else {
		// mTouchOffset.first = mBitmapRect.exactCenterX();
		// mTouchOffset.second = mBitmapRect.exactCenterY();
		// }
